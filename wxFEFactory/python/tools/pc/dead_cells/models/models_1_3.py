...
class Progress(Model):
    ...
    normal_chests_opened = Field(0x44, label="普通宝箱开启数")
    cursed_chests_opened = Field(0x48, label="诅咒宝箱开启数")
    secret_portals_opened = Field(0x4C, label="秘密区域开启数")
    challenges_completed = Field(0x50, label="挑战完成数")
    challenges_failed = Field(0x54, label="挑战失败数")
    slot_22 = Field(0x58, label="Slot 22")
    untouchable_doors_completed = Field(0x5C, label="无伤击杀门完成数")
    untouchable_doors_failed = Field(0x60, label="无伤击杀门失败数")
    timed_door_challenges_completed = Field(0x64, label="限时门通过数")
    timed_door_challenges_failed = Field(0x68, label="限时门失败数")
    slot_27 = Field(0x6C, label="Slot 27")
    deaths_by_trap = Field(0x70, label="陷阱死亡数")
    killed_in_a_challenge_room = Field(0x74, label="挑战门死亡数")
    deaths_by_falling = Field(0x78, label="跌落死亡数")
    deaths_by_infection = Field(0x7C, label="反弹死亡数")
    deaths_by_suicide = Field(0x80, label="自杀死亡数")
    cells_lost = Field(0x84, label="细胞丢失数")
...
class Player(Model):
    ...
    # 武器槽
    inventory = ArrayField((0x300, 0x4, 0x330, 0x4, 0x8, 0x10), 50, ModelPtrField(0, Weapon))
