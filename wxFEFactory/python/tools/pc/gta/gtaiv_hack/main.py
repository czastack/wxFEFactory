...
from lib.hack.forms import Group, StaticGroup, ModelInput, ModelCoordWidget, ModelSelect
from lib.hack.utils import u32_bytes
...
class Main(BaseGTATool):
    ...
    def init_addr(self):
        """初始化地址信息"""
        self.MODULE_BASE = self.handler.base_addr - 0x400000
        ...
        self.FUNCTION_FIND_NATIVE_ADDRESS = (
            b'\x55\x8B\xEC\x83\xEC\x08\x56\xC7\x45\xF8' + u32_bytes(address.FindNativeAddress) +
    ...
    def get_selected_vehicle_list(self):
        """当前刷车器活动的载具模型列表"""
        return datasets.VEHICLE_LIST[self.vehicle_model_book.index][1]
    ...
    def create_explosion(self, coord, uiExplosionType=None,
            fRadius=0, bSound=True, bInvisible=False, fCameraShake=0):
        """产生爆炸"""
